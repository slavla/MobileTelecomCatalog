...
export function getCatalog(customerType) {
    console.log('getCatalog called with customerType:', customerType);
    const baseCatalog = [
    ...
    baseCatalog.forEach(service => {
        service.businessPrice = service.residentialPrice * 0.8; // Business is 20% off
        service.partnerPrice = service.residentialPrice * 0.7;  // Partner is 30% off
    });

    // Return catalog with appropriate pricing
    const catalog = baseCatalog.map(service => ({
        ...service,
        price: customerType === 'Business' ? service.businessPrice :
               customerType === 'Partner' ? service.partnerPrice :
               service.residentialPrice
    }));

    console.log('Catalog generated:', catalog);
    return catalog;
}
